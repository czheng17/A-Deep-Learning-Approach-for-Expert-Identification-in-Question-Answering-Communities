...
x_train_answer1= torch.from_numpy(Dataset_handle.load_data_answer1(cnn_word_vocab,raw_data,number_questions))
x_train_answer2= torch.from_numpy(Dataset_handle.load_data_answer2(cnn_word_vocab,raw_data,number_questions))
...
test_answer1 = torch.from_numpy(Dataset_handle.load_data_answer1(cnn_word_vocab,test_raw_data,number_questions_val))
test_answer2 = torch.from_numpy(Dataset_handle.load_data_answer2(cnn_word_vocab,raw_data,number_questions_val))

# load the cnn word embedding
vectors_ques = Dataset_handle.load_vectors()
print("Load done...")

val_file = '../../8_21_stack_overflow_data/qa/test1_use.txt'
precision = '../../8_21_stack_overflow_data/qa/test1_use_change_margin.acc'
...
#     shuffle=True,
# )
def get_batch(sentence, answer1, answer2, i, evaluation=False):
    # seq_len = min(args.bptt, len(source) - 1 - i)
    data = Variable(sentence[i:i+args.batch_size], volatile=evaluation)
    target1 = Variable(answer1[i:i+args.batch_size],volatile=evaluation)
    target2 = Variable(answer2[i:i+args.batch_size],volatile=evaluation)
...
for epoch in range(args.epochs):
    for step in range(0, len(x_train_ques),args.batch_size):
        t_x, t_y1, t_y2 = get_batch(x_train_ques, x_train_answer1, x_train_answer2, step)


        if args.cuda:
                t_x, t_y1, t_y2 = t_x.cuda(), t_y1.cuda(), t_y2.cuda()

        sentence_output = cnn(t_x)
        sentence_output_answer1 = cnn(t_y1)
        sentence_output_answer2 = cnn(t_y2)
        ...
        # cos simlilaity
        matmul_o1 = torch.nn.functional.cosine_similarity(sentence_output,sentence_output_answer1,dim=1, eps=1e-6)
        matmul_o2 = torch.nn.functional.cosine_similarity(sentence_output, sentence_output_answer2, dim=1, eps=1e-6)
    ...
    if epoch % 10 == 0 and epoch != 0 :
        ...
        for val_step in range(0, number_questions_val,args.batch_size ):
            t_x, t_y1, t_y2 = get_batch(test_ques, test_answer1, test_answer2, val_step)
            if args.cuda:
                    t_x, t_y1, t_y2 = t_x.cuda(), t_y1.cuda(), t_y2.cuda()
