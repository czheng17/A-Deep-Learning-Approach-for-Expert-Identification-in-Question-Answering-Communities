...
def build_vocab():
    ...
    vocab['UNKNOWN'] = code
    code += 1
    for line in open('../../8_21_stack_overflow_data/qa/vector_q_a_baseline.txt'):
...
def read_raw():
    raw = []
    for line in open('../../8_21_stack_overflow_data/qa/train_use.txt'):
        items = line.strip().split(' ')
        if items[0] == '0':
            raw.append(items)
    # print raw
...
def test_read_raw():
    raw = []
    for line in open('../../8_21_stack_overflow_data/qa/test1_use.txt'):
        items = line.strip().split(' ')
        raw.append(items)
    # print raw
    return raw
...
def load_data_answer1(vocab, raw, size):
    x_train_answer = []
    for i in range(0, size):
        items = raw[i]
        x_train_answer.append(encode_sent(vocab, items[3], 50)) #150 a_b_<a> total 150_
    # return x_train_ques
    return np.array(x_train_answer)

def load_data_answer2(vocab, raw, size):
    x_train_answer = []
    for i in range(0, size):
        items = raw[i]
        x_train_answer.append(encode_sent(vocab, items[4], 50)) #150 a_b_<a> total 150_
    # return x_train_ques
    return np.array(x_train_answer)

def load_data_answer_test(vocab, raw, size):
    x_train_answer = []
    for i in range(0, size):
        items = raw[i]
        x_train_answer.append(encode_sent(vocab, items[4], 50)) #150 a_b_<a> total 150_
    # return x_train_ques
    return np.array(x_train_answer)
# print load_data_answer_test(build_vocab(), read_raw(), 10)
